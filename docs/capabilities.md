...
| ETCLOVG harness | **Partial** | `EtclovgHarness.runAsync`, `EtclovgConfig`, and `EtclovgResult`; harness unit and end-to-end tests | The concerns are integrated, but there is no durable request/result, checkpoint, or replay contract. | HAR-01, HAR-02 |
...
| Capability | Status | Source and tests | Current boundary | Roadmap |
|---|---|---|---|---|
| Key/value, working, episodic, and tiered memory | **Partial** | Memory contracts in `Nao.Agents`; in-memory, file, and ADO implementations in `Nao.Persistence`; memory/persistence tests | Scope is caller-supplied; stores do not independently enforce tenant or user authorization. | MEM-01, RUN-01 |
...
| Semantic memory | **Partial** | Functional `SemanticMemory` and `EmbeddingProvider` records, simple embedding provider, in-memory/file/ADO implementations; semantic-memory tests | File and ADO stores scan embeddings in process; no indexed vector backend, backend ACL filters, model-version isolation, or re-embedding migration exists. | KNO-05, KNO-06 |
| Graph memory | **Partial** | Functional `GraphMemory`, `InMemoryGraphMemory`, and `PersistentGraphMemory`; rich-store tests | Relation removal is not implemented, node removal can leave dangling relations, and persistent removal lacks complete relation events. | GRA-01 |
...
| Dataset runner and reports | **Partial** | `EvalRunner.runCaseAsync`, `runDatasetAsync`, and `EvalReport`; runner tests | Runner invokes agents with `AgentContext.allowAll`, bypasses the harness, does not use per-case timeout, and sequential stop-on-first-failure is incomplete. | EVAL-02, HAR-01 |
| Traces, metrics, journals, and resilience | **Implemented** primitive | Tracer, metrics collectors, journals, retries, circuit breaker, and fallback; observability tests | There is no standard cross-component schema or guaranteed propagation through all external boundaries. | OBS-01 |
...
| Platform vocabulary, ownership, and failure taxonomy | **Partial** | `PlatformErrorCategory`, tool/harness mappings, `SessionDeletion`, runtime isolation tests, `docs/architecture.md`, and documentation validation | Vocabulary, ownership, trust, event authority, and retry rules are defined. Session destruction coordinates conversation, memory, directory, and runtime-state deletion. Other durable stores plus provider, persistence, and Orleans error mappings remain incomplete. | FND-03 |
