...
| ETCLOVG harness | **Partial** | Immutable `ExecutionRequest`, grouped `ExecutionResult`, `ExecutionTerminalStatus`, caller-cancellable `EtclovgHarness.runAsync`, and typed durable boundary checkpoints; harness unit, production/evaluation parity, persistence parity, and end-to-end tests | Supported production and evaluation execution enters the harness and records accepted, execution-started, and terminal phases. Resume, idempotency, side-effect intent, and executable replay remain HAR-02 work. | HAR-01, HAR-02 |
