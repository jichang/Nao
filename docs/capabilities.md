...
| ETCLOVG harness | **Partial** | Immutable `ExecutionRequest`, `EtclovgHarness.runAsync`, `EtclovgConfig`, and `EtclovgResult`; harness unit and end-to-end tests | The concerns are integrated and request identity is explicit, but there is no durable result, checkpoint, or replay contract. | HAR-01, HAR-02 |
