...
module JsonText =
    ...
    let tryExtractBalancedObject (text: string) : string option =
        ...
        if start < 0 then
            ...
            if finish < 0 then
                ...
                Some(text.Substring(start, finish - start + 1))
        ...
        if not (trimmed.Contains("{{", StringComparison.Ordinal)) then
            ...
            while index < trimmed.Length && not invalid do
                ...
                if inString then
                    ...
                    let count =
                        if index + 1 < trimmed.Length && trimmed[index + 1] = '{' then
                            2
                        else
                            1
            ...
            if invalid || inString || not stack.IsEmpty then
                trimmed
            else
                output.ToString()
    ...
    let tryCompleteDelimiters (text: string) : string option =
        ...
        if start < 0 then
            ...
            if invalid || inString || List.isEmpty stack then
                None
            else
                Some(candidate + String(Array.ofList stack))
    ...
    let normalizeObjectLikeArrays (json: string) =
        let nextNonWhitespace start =
            ...
            while index < json.Length && Char.IsWhiteSpace(json[index]) do
                index <- index + 1
        ...
        let startsWithProperty start =
            ...
            if index >= json.Length || json[index] <> '"' then
                false
            else
                index <- index + 1
                let mutable escaped = false
