...
module ProviderFactory =
    ...
    let create (providerType: ProviderType) : LlmProvider =
        ...
        | OpenAI config -> openAi "OpenAI" config.BaseUrl config.Model (optionalKey config.ApiKey) config.TimeoutSeconds
        | DeepSeek config ->
            openAi "DeepSeek" config.BaseUrl config.Model (optionalKey config.ApiKey) config.TimeoutSeconds
        | Kimi config -> openAi "Kimi" config.BaseUrl config.Model (optionalKey config.ApiKey) config.TimeoutSeconds
        | Vllm config -> openAi "vLLM" config.BaseUrl config.Model config.ApiKey config.TimeoutSeconds
        | LlamaCpp config -> openAi "llama.cpp" config.BaseUrl config.Model None config.TimeoutSeconds
        | Ollama config -> OllamaProvider.create config
        | Anthropic config -> AnthropicProvider.create config
