...
type AnthropicProvider(config: AnthropicConfig, ?httpHandler: HttpMessageHandler) =
    ...
    let finishReason reason =
        ...
        | value when String.IsNullOrWhiteSpace value ->
            raise (JsonException("stop_reason must be a non-empty string."))
        | value -> value

    let buildRequestBody (conversation: Conversation) (options: CompletionOptions) streaming =
        let systemPrompt =
            conversation
            |> List.choose (fun message -> if message.Role = Role.System then Some message.Content else None)
            |> String.concat "\n\n"
        let messages =
            conversation
            |> List.choose (fun message ->
                if message.Role = Role.System then None
                else Some((if message.Role = Role.User then "user" else "assistant"), message.Content))
            |> List.toArray
        AnthropicDto.serializeRequest
            config.Model (options.MaxTokens |> Option.defaultValue 4096) options.Temperature streaming
    ...
    let parseStreamEvent (json: string) =
        ...
        | "message_start" ->
            if isNull event.Message || isNull event.Message.Usage || not event.Message.Usage.InputTokens.HasValue then
        ...
        | "content_block_delta" ->
            if isNull event.Delta || event.Delta.Type <> "text_delta" || isNull event.Delta.Text then
                raise (JsonException("content_block_delta requires a text_delta with text."))
            event.Delta.Text, None, None, None
        | "message_delta" ->
            if isNull event.Delta || isNull event.Usage || not event.Usage.OutputTokens.HasValue then
            ...
            task {
                try
                    ...
                    if response.IsSuccessStatusCode then
                        ...
                        return { Content = sprintf "Error: %d - %s" (int response.StatusCode) responseBody; FinishReason = "error"; TokensUsed = None; Usage = None }
                with ex ->
        ...
        member _.StreamAsync conversation options onChunk : Task<CompletionResult> =
            task {
                try
                    ...
                    request.Content <- new StringContent(buildRequestBody conversation options true, Encoding.UTF8, "application/json")
                        ...
                        while reading do
                            ...
                            if isNull line then
                                ...
                                if trimmed.StartsWith("data:") then
                                    let delta, eventReason, eventInputTokens, eventOutputTokens = parseStreamEvent (trimmed.Substring(5).Trim())
