...
module ResourceUsage =
    let check (limits: ResourceLimits) (usage: ResourceUsage) : LimitExceeded option =
        if usage.ElapsedTime > limits.MaxDuration then
            Some LimitExceeded.Duration
        elif usage.LlmCalls > limits.MaxLlmCalls then
            Some LimitExceeded.LlmCalls
        elif usage.TotalTokens > limits.MaxTotalTokens then
            Some LimitExceeded.TotalTokens
        elif usage.EstimatedCostUsd > limits.MaxCostUsd then
            Some LimitExceeded.Cost
        elif usage.ToolCalls > limits.MaxToolCalls then
            Some LimitExceeded.ToolCalls
        else
            None
