...
type Pipeline = { Stages: Agent list }

module Pipeline =

    /// Create a pipeline from an ordered list of agents.
    let create (stages: Agent list) = { Stages = stages }
        ...
        task {
            ...
            | Some graph ->
                ...
                | Error(StepLimitReached(maxSteps, _)) ->
                    return
                        raise (
                            InvalidOperationException(sprintf "Pipeline exceeded its graph limit of %d steps." maxSteps)
                        )
        }
