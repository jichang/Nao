...
type OrchestratorConfig = { Id: string; Name: string; Description: string; Priority: int; Responsibilities: string list; Contract: AgentContract; Provider: LlmProvider; Tools: Tool list; SubAgents: Agent list; Prompt: Prompt; Options: CompletionOptions; MaxRounds: int; Bus: EventBus; Scope: EventScope }
...
type OrchestratorDefinition = { GenerateReasoningPrompt: Conversation -> Task<Conversation>; PrepareRound: (Conversation -> Task<OrchestratorRound>) option; ResponseProtocol: Nao.Protocols.ResponseProtocol<AgentAction> option; ParseActions: string -> AgentAction list; ValidateResponse: string -> string option; BuildRepairMessage: string -> string }
...
module OrchestratorDefinition =
    let create generateReasoningPrompt =
        { GenerateReasoningPrompt = generateReasoningPrompt
          ...
          BuildRepairMessage = fun error -> sprintf "[System]: Your previous response was invalid: %s. Please re-send a corrected response." error }

/// Runs a functional planning definition inside the bounded orchestration loop.
module Orchestrator =
    let createWithProtocol (toolProtocol: ToolProtocol) (config: OrchestratorConfig) (definition: OrchestratorDefinition) : Agent =
        let id = config.Id
        ...
        let report (signal: ProgressSignal) =
            EventBus.publishAsync (NaoEvent.TurnProgress(eventScope, signal)) eventBus |> ignore
            ...
            task {
                ...
                let startLlmSpan (attempt: int) (messageCount: int) : (Tracer * Span) option =
                    ...
                    | Some (tracer, parent) ->
                        ...
                        Some (tracer, span)
                    | None -> None
                let endLlmSpan (span: (Tracer * Span) option) (status: SpanStatus) (outputLength: int) (elapsedMs: int64) =
                    match span with
                    | Some (tracer, child) ->
                ...
                try
                    ...
                    endRoundSpan roundSpan SpanStatus.Ok
        ...
        let runCore (agentContext: AgentContext) (input: string) : Task<string> =
            task {
                ...
                while not finished && rounds < config.MaxRounds do
                    ...
                    // parses the response.
                    let! response, parseActions = reasonAsync conversation (rounds + 1) successfulToolCalls
                            ...
                            if not finished then
                                ...
                                | InvokeTool (toolName, toolInput) ->
                                    ...
                                    | Some _ ->
                                          ...
                                          | Some metrics -> metrics.RecordToolCall toolName toolExecution.DurationMs toolExecution.Success
                                          | None -> ()
                                          if toolExecution.Success then
                                              ...
                                              let durationMs = toolExecution.DurationMs.ToString(Globalization.CultureInfo.InvariantCulture)
                                              ...
                                              | None -> ()
                                              endToolSpan toolSpan SpanStatus.Ok (Map.ofList [ "result.length", string toolResult.Length ])
                                          else
                                              let failure =
                                                  toolExecution.Failure
                                                  |> Option.defaultValue
                                                      { Kind = ToolFailureKind.Execution
                                                        Message = toolExecution.Error |> Option.defaultValue "Tool invocation failed."
                                                        Retryable = false }
                                              let guidance =
                                              ...
                                              endToolSpan toolSpan (SpanStatus.Error (sprintf "%A failed for %s" failure.Kind toolName)) Map.empty
                                    | None ->
                                ...
                                | DelegateToAgent (agentId, agentInput) ->
                                    ...
                                    if String.Equals(agentId, config.Id, StringComparison.OrdinalIgnoreCase) then
                                        ...
                                        | Some agent ->
                                            ...
                                            finished <- true
                                        | None ->
                                            let err = sprintf "Agent '%s' not found. Available agent identifiers: %s" agentId (config.SubAgents |> List.map (fun a -> a.Metadata.Id) |> String.concat ", ")
                                            conversation <- conversation @ [ { Role = User; Content = sprintf "[Error]: %s" err } ]
