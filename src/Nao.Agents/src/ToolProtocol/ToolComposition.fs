...
and ToolStep =
    ...
    static member WithTransform (inputFn: string -> string) (outputFn: string -> string) (step: ToolStep) =
        { step with
            InputTransform = Some inputFn
            OutputTransform = Some outputFn }
...
module ToolComposer =
    ...
    let private failed message retryable =
        { Success = false
          Output = ""
          Error = Some message
          Failure =
    ...
    let private executeStep
        (context: AgentContext)
        (protocol: ToolProtocol)
        (step: ToolStep)
        (input: string)
        : Task<ToolInvocationResult> =
        task {
            ...
            let! result =
                task {
                    ...
                    with :? TimeoutException ->
                        return failed (sprintf "Tool '%s' timed out." step.ToolName) true
                }
    ...
    let rec executeAsync
        (context: AgentContext)
        (protocol: ToolProtocol)
        (composition: ToolComposition)
        (input: string)
        : Task<CompositionResult> =
        task {
            ...
            | ToolComposition.Adaptive(step, router) ->
                ...
                sw.Stop()
                let finalOutput = if continueLoop then "" else currentInput
