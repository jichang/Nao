...
type MemoryToolConfig =
    { SearchEnabled: bool; RememberEnabled: bool; ForgetEnabled: bool; MaxSearchResults: int; MaxEntryChars: int }
    static member Default = { SearchEnabled = true; RememberEnabled = true; ForgetEnabled = false; MaxSearchResults = 5; MaxEntryChars = 2000 }
...
type private MemoryForgetInput = { Key: string; Reason: string; ConfirmedByUser: bool }
...
module private MemoryToolJson =
    ...
    let decodeSearch value =
        parseObject value (fun root ->
            ...
            | Ok query, Ok intent, Ok tags ->
                ...
                | false, _ -> Ok { Query = query; Intent = intent; Tags = tags; Limit = None }
                | true, limit when limit.ValueKind = JsonValueKind.Number ->
                    match limit.TryGetInt32() with
                    | true, count when count > 0 -> Ok { Query = query; Intent = intent; Tags = tags; Limit = Some count }
    ...
    let decodeRemember value =
        parseObject value (fun root ->
            ...
            | Ok key, Ok memoryValue, Ok tags -> Ok { Key = key; Value = memoryValue; Tags = tags }
            | Error error, _, _
            | _, Error error, _
            | _, _, Error error -> Error error)
    ...
    let decodeForget value =
        parseObject value (fun root ->
            ...
            | Ok key, Ok reason, Ok confirmed -> Ok { Key = key; Reason = reason; ConfirmedByUser = confirmed }
            | Error error, _, _
            | _, Error error, _
            | _, _, Error error -> Error error)
    ...
    let encodeSearchResult (intent: string) (entries: MemoryEntry list) =
        try
            ...
            for entry in entries do
                ...
                for tag in entry.Tags do writer.WriteStringValue tag
...
module private MemorySearch =
    ...
    let rank (input: MemorySearchInput) (entry: MemoryEntry) =
        ...
        let tagScore = Set.intersect wanted (entry.Tags |> String.concat " " |> words) |> Set.count |> (*) 2
