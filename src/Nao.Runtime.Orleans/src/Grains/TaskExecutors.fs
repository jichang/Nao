...
/// Built-in executor for "tool" tasks: runs a registered compiled tool in the background.
        ...
        member _.ExecuteAsync(ctx: TaskExecutionContext) : Task<TaskOutcome> =
            task {
                ...
                | Some workspace ->
                    ...
                    match workspace.Tools |> List.tryFind (fun tool -> tool.Name = n && VersionRef.matches ver tool.Version) with
                    | None ->
                        return { Summary = sprintf "[Error] Async tool '%s' not found" toolName; ResultFileIds = [] }
                    | Some tool ->
                        ctx.Report 0.1 (sprintf "Running %s" toolName)
                        // Files generated by this background tool are shared with the parent
                        // session; permissions/spawn default to allow/none (it runs headless).
                        let toolContext =
                            { ToolContext.allowAll with
                                SessionKey = ctx.ParentKey
                                FilesKey = ctx.ParentKey
                                TurnId = ctx.TurnId }
                        let! output = tool.InvokeAsync(toolContext, input)
                        ctx.Report 1.0 "Completed"
