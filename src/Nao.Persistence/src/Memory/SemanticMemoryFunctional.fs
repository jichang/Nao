...
module InMemorySemanticMemory =
    let create (embeddingProvider: EmbeddingProvider) : SemanticMemory =
        ...
        { StoreAsync = fun agentId key content ->
              task {
          ...
          RetrieveAsync = fun agentId query topK ->
              task {
                  ...
                  | true, entries ->
                      return
                          entries
                          |> List.map (fun entry -> entry, SemanticSimilarity.cosineSimilarity queryEmbedding entry.Embedding)
                          |> List.sortByDescending snd
                          |> List.truncate topK
                          |> List.map fst
                  | false, _ -> return []
              }
          RemoveAsync = fun agentId key ->
              task {
                  match store.TryGetValue agentId with
...
module FileSemanticMemory =
    let create (embeddingProvider: EmbeddingProvider) (baseDir: string) : SemanticMemory =
        ...
        let load agentId = FileJson.read<Dto.SemanticEntryDto list> (agentFile agentId) []
        ...
        { StoreAsync = fun agentId key content ->
              task {
                  ...
                  let entry: SemanticEntry =
                      { Key = key
                        ...
                        Tags = [] }
                  lock sync (fun () ->
                      save agentId (Dto.toSemanticDto entry :: (load agentId |> List.filter (fun item -> item.Key <> key))))
              }
          RetrieveAsync = fun agentId query topK ->
              task {
                  let! queryEmbedding = embeddingProvider.EmbedAsync query
          ...
          RemoveAsync = fun agentId key ->
              task {
                  lock sync (fun () ->
                      save agentId (load agentId |> List.filter (fun item -> item.Key <> key)))
              } }
