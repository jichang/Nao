...
type ToolPermissionClassifyTests() =
    ...
    member _.ConvertDocument_IsNotStaticallyClassified() =
        ...
        Assert.AreEqual(0, List.length (ToolPermissions.classifyAll AgentContext.allowAll "convert_document" """{"source":"report.md","target":"pdf"}"""))

    [<TestMethod>]
    member _.SingleResourceTool_StillYieldsOneAccess() =
        let accesses = ToolPermissions.classifyAll AgentContext.allowAll "read_file" """{"path":"notes.txt"}"""
    ...
    member _.UnknownTool_YieldsNoAccess() =
        Assert.AreEqual(0, List.length (ToolPermissions.classifyAll AgentContext.allowAll "calculator" "1+1"))
