...
type ActionBlockParseTests() =
    ...
    let makeOrchestrator provider tools =
        NaoOrchestrator({ Id = { Name = "orchestrator"; Description = "test orchestrator" }; Provider = provider; Tools = tools; SubAgents = [ converterAgent ]; Prompt = Prompt.Empty; Options = CompletionOptions.Default; MaxRounds = 5; Bus = EventBus.none; Scope = EventScope.Empty })
